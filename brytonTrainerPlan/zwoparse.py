...
def parse_power(node):
    ...
    if min_intensity is None and max_intensity is None:
        min_intensity = float(target_intensity) - 0.03
        max_intensity = float(target_intensity) + 0.03
...
def parse_textevents(node, segment):
    for textevent in node.iter("textevent"):
        timeoffset = int(textevent.get("timeoffset")) if textevent.get("timeoffset") != None else 0
...
def parse(xmlstring, minduration=0):
    ...
    description = root.find("description").text if root.find("description")!= None else ""
